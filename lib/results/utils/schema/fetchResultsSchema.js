...
const fetchResultSchema = Joi.object({
        //schoolID: Joi.string(),
        _id: Joi.string(),
        section: Joi.string(),
        classID: Joi.string(),
...
const fetchResult = async function(Request, Response, next){
    try {
        const error = await fetchResultSchema.validateAsync(Request.query);
        next();
    } catch (error) {
