/**
 * @name parent
 * @description parent Fetch Result flow
 * @author Vijaykumar
 * @version 1.0
 */
...
/* Fetch Result */
var parentFetchResult = async function (Request, Response, connection, ACCESS_TOKEN, ACCESS_TOKEN_DECODED) {
    try {
        ...
        /* Fetch Managemnet */
        var parentData = ACCESS_TOKEN_DECODED.id       
        var fetchparentOuput = await restApi(`${process.env.Mangament_URL}?_id=${parentData}`,ACCESS_TOKEN)
        //Error Handle for DB Error
        if (fetchparentOuput.error != null || fetchparentOuput.response.payLoad.length == 0 ) {
            throw {"code": "FST105F", "message": "Parent Fetch - DB Error", "statusCode": 500}
        }

        ResultQuery["schoolID"] = fetchparentOuput.response.payLoad[0].schoolID
...
module.exports = parentFetchResult