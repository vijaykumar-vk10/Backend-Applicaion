/**
 * @name school
 * @description school Fetch Result flow
 * @author Vijaykumar
 * @version 1.0
 */
...
/* Fetch Result */
var schoolFetchResult = async function (Request, Response, connection, ACCESS_TOKEN, ACCESS_TOKEN_DECODED) {
    try {
        ...
        /* Fetch Managemnet */
        var schoolData = ACCESS_TOKEN_DECODED.id       
        var fetchschoolOuput = await restApi(`${process.env.Mangament_URL}?_id=${schoolData}`,ACCESS_TOKEN)
        //Error Handle for DB Error
        if (fetchschoolOuput.error != null || fetchschoolOuput.response.payLoad.length == 0 ) {
            throw {"code": "FST105F", "message": "School Fetch - DB Error", "statusCode": 500}
        }

        ResultQuery["schoolID"] = fetchschoolOuput.response.payLoad[0].schoolID
        ...
        /* Build Response */
        var responseCanister = new Canister("registration","createAssessment")
...
module.exports = schoolFetchResult